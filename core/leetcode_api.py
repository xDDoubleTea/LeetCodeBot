...
class LeetCodeAPI:
    ...
    async def fetch_all_problems(
        ...
        logger.info("Fetching all problems from GitHub")
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                logger.debug("All Problems JSON: %s", validated_response_json)
                return await self.parse_all_problem_response(validated_response_json)

    async def fetch_problem_by_id(self, id: int) -> ProblemWithTags:
        logger.info(f"Fetching problem with ID {id}")
        async with aiohttp.ClientSession() as session:
            async with session.get(url=f"{self._base_url}/problem/{id}") as response:
                validated_response_json = await self._validate_response(
                    response, f"Failed to fetch problem with ID {id}"
                )
                logger.info(f"Fetched problem with ID {id} successfully")
                logger.debug(f"Problem with ID {id} JSON: %s", validated_response_json)
                return await self.parse_single_problem_response(validated_response_json)

    async def fetch_problem_by_slug(self, slug: str) -> ProblemWithTags:
        logger.info(f"Fetching problem with slug {slug}")
        async with aiohttp.ClientSession() as session:
            async with session.get(url=f"{self._base_url}/problem/{slug}") as response:
                validated_response_json = await self._validate_response(
                    response, f"Failed to fetch problem with slug {slug}"
                )
                logger.info(f"Fetched problem with slug {slug} successfully")
                logger.debug(
